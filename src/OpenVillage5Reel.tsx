...
import plan from "./r2068-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * OV5_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * OV5_FPS);
...
export const makeOV5 = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * OV5_FPS);
  const Reel: React.FC = () => (
...
export const OV5 = REEL_PLANS.map(makeOV5);
