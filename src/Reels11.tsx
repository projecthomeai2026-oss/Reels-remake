...
import plan from "./reel11-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * REELS11_FPS);
...
// Задержка субтитров: whisper ставит слова чуть раньше реального звука —
// сдвигаем субтитры позже, чтобы они не убегали вперёд (просьба Романа 2026-07-24)
const CAPTION_DELAY_MS = 180;
...
const buildCaptions = (items: TimelineItem[]): Caption[] => {
  ...
  for (const item of items) {
    ...
    for (const w of words) {
      const capLimit = item.captionToMs ?? item.toMs;
      if (w.startMs >= item.fromMs - 60 && w.startMs < capLimit) {
        const startMs =
          item.globalStartMs +
  ...
  // whisper сбивает слова к началу фразы (у слова бывает 60 мс), из-за чего
  // караоке-подсветка гаснет раньше звука. Тянем каждое слово до начала
  // следующего в той же сцене — подсветка идёт непрерывно, в темп речи
  captions.sort((a, b) => a.startMs - b.startMs);
  for (let i = 0; i < captions.length - 1; i++) {
    const gap = captions[i + 1].startMs - captions[i].startMs;
    // не растягивать через большие паузы между сценами (>900 мс)
    if (gap > 0 && gap < 900) {
      captions[i].endMs = captions[i + 1].startMs;
      captions[i].timestampMs = (captions[i].startMs + captions[i].endMs) / 2;
    }
  }
  return captions;
};
  ...
  return (
    <AbsoluteFill>
      {pages.map((page, index) => {
        ...
        // страница живёт до следующей (в пределах сцены), чтобы субтитры не
        // пропадали в паузах речи; на хвосте — до +3.5с
    ...
    <AbsoluteFill style={{ overflow: "hidden" }}>
      <OffthreadVideo
        ...
        style={{
          ...
          height: "100%",
          objectFit: "cover",
          // хук без цветофильтра — тот же натуральный грейд, что и тело (просьба Романа)
...
const HookOverlay: React.FC<{ animateIn?: boolean }> = ({ animateIn }) => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      ...
      />
      {/* тёплая вспышка на входе убрана — Роману не нравился цветофильтр на входе */}
    </AbsoluteFill>
  );
};
...
const EndCTA: React.FC = () => {
  ...
  return (
    <AbsoluteFill
      style={{ justifyContent: "flex-start", alignItems: "center", paddingTop: 250, pointerEvents: "none" }}
    >
      <div
        style={{
          display: "flex",
...
export const makeReel = (reel: ReelPlan) => {
  ...
  const duration = Math.ceil((totalMs / 1000) * REELS11_FPS);
...
export const REELS11 = REEL_PLANS.map(makeReel);
