...
import plan from "./reel7-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * REELS7_FPS);
...
// Эффектный цветной опенер (без ч/б и глич-эффектов, стиль reel9): тёплая
// вспышка на входе, въезжающие киношные полосы, усиленная виньетка.
// Цвет качает SegmentVideo.
...
export const makeReel = (reel: ReelPlan) => {
  ...
  const duration = Math.ceil((totalMs / 1000) * REELS7_FPS);
...
export const REELS7 = REEL_PLANS.map(makeReel);
