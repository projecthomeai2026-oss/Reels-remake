...
import plan from "./flag-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * FLAG_FPS);
...
const SegmentVideo: React.FC<{ item: TimelineItem; accents?: Accent[] }> = ({ item, accents = [] }) => {
  ...
  const baseZoom = 1; // без зум-эффектов (по просьбе Романа)
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", opacity: Math.min(fadeIn, fadeOut) }}>
      <OffthreadVideo
        ...
        toneFrequency={item.rate !== 1 ? 1 / item.rate : undefined}
        volume={item.volume}
        style={{ width: "100%", height: "100%", objectFit: "cover", transform: `scale(${zoom})`, filter: "contrast(1.04) saturate(1.06) brightness(1.05)" }}
      />
    </AbsoluteFill>
  );
};
...
const SeriesBadge: React.FC<{ series: Series }> = ({ series }) => (
  <AbsoluteFill style={{ pointerEvents: "none" }}>
    {/* локация справа (бренд уже на флаге и в хуке) */}
    <div style={{
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * FLAG_FPS);
...
export const makeFlag = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * FLAG_FPS);
  const Reel: React.FC = () => (
...
export const FLAG = REEL_PLANS.map(makeFlag);
