...
import plan from "./pool97-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * POOL97_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * POOL97_FPS);
...
export const makePool97 = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * POOL97_FPS);
  const Reel: React.FC = () => (
...
export const POOL97 = REEL_PLANS.map(makePool97);
