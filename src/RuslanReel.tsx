...
const msToFrame = (ms: number) => Math.round((ms / 1000) * RUSLAN_FPS);
const XFADE_MS = 350;
...
const CineFrame: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      <AbsoluteFill style={{ boxShadow: "inset 0 0 180px rgba(0,0,0,0.26)" }} />
...
const HeroPhrase: React.FC<{ hero: Hero }> = ({ hero }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const durF = Math.round((hero.durMs / 1000) * RUSLAN_FPS);
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", pointerEvents: "none", opacity: op }}>
      ...
      <div style={{ textAlign: "center", transform: `translateY(${(1 - enter) * 40}px) scale(${interpolate(enter, [0, 1], [0.9, 1])})` }}>
        <div style={{ fontFamily, fontSize: 26, fontWeight: 800, letterSpacing: "0.34em", textTransform: "uppercase", color: "#FFE500", marginBottom: 20 }}>{series.sub}</div>
        <div style={{ fontFamily, fontSize: 150, fontWeight: 900, fontStyle: "italic", color: "#fff", lineHeight: 0.9, textShadow: "0 12px 44px rgba(0,0,0,0.7)", textTransform: "uppercase" }}>
          {series.title.split("\n").map((l, i) => <div key={i}>{l}</div>)}
        </div>
        <div style={{ fontFamily, fontSize: 32, fontWeight: 900, textTransform: "uppercase", color: "#0a0a0a", background: "#FFE500", display: "inline-block", padding: "10px 26px", borderRadius: 12, marginTop: 26, rotate: "-2deg", boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>🪟 УМНЫЕ ОКНА</div>
      </div>
    </AbsoluteFill>
  );
};
...
const Badges: React.FC = () => (
  <AbsoluteFill style={{ pointerEvents: "none" }}>
    <div style={{ position: "absolute", top: 158, right: 40, fontFamily, fontSize: 25, fontWeight: 900, color: "#0a0a0a", background: "#FFE500", padding: "7px 15px", borderRadius: 11, textTransform: "uppercase", boxShadow: "0 4px 14px rgba(0,0,0,0.4)" }}>🪟 РАЙТ ОКНА</div>
  </AbsoluteFill>
);
...
const Sticker: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * RUSLAN_FPS);
  const out = interpolate(frame, [durF - 8, durF], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
export const makeRuslan = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * RUSLAN_FPS);
  const Reel: React.FC = () => (
...
export const RUSLAN = REEL_PLANS.map(makeRuslan);
