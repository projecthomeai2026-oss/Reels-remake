...
import plan from "./remont-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * REMONT_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * REMONT_FPS);
...
export const makeRemont = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * REMONT_FPS);
  const Reel: React.FC = () => (
...
export const REMONT = REEL_PLANS.map(makeRemont);
