...
import plan from "./andrey-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * ANDREY_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * ANDREY_FPS);
...
export const makeAndrey = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * ANDREY_FPS);
  const Reel: React.FC = () => (
...
export const ANDREY = REEL_PLANS.map(makeAndrey);
