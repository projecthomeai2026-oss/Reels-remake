...
import plan from "./pool4-plan.json";
...
type Series = { n: number; total: number; theme: string };
type Accent = { atMs: number; durMs: number };
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * POOL_FPS);
...
const SegmentVideo: React.FC<{ item: TimelineItem; accents?: Accent[] }> = ({ item, accents = [] }) => {
  ...
  for (const a of accents) {
    if (tMs >= a.atMs - 400 && tMs <= a.atMs + a.durMs + 400) {
      ...
      punch = Math.max(punch, interpolate(local, [-400, 300, a.durMs - 200, a.durMs + 400], [0, 0.12, 0.12, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.inOut(Easing.quad) }));
    }
  }
...
const SeriesCover: React.FC<{ series: Series }> = ({ series }) => {
  ...
  const op = interpolate(frame, [0, 10, 74, 86], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", pointerEvents: "none", opacity: op }}>
      {/* затемняющая подложка для читаемости заставки */}
      <AbsoluteFill style={{ background: "rgba(0,0,0,0.42)" }} />
      <div style={{ textAlign: "center", transform: `translateY(${(1 - enter) * 40}px)` }}>
        <div style={{ fontFamily, fontSize: 26, fontWeight: 700, letterSpacing: "0.42em", textTransform: "uppercase", color: "#FFDD00", marginBottom: 20 }}>
          Устройство бассейна
        </div>
        <div style={{ fontFamily, fontSize: 128, fontWeight: 900, fontStyle: "italic", color: "#fff", lineHeight: 0.9, textShadow: "0 10px 40px rgba(0,0,0,0.6)" }}>
          СЕРИЯ {series.n}
        </div>
        <div style={{ fontFamily, fontSize: 30, fontWeight: 600, letterSpacing: "0.05em", color: "rgba(255,255,255,0.85)", marginTop: 8 }}>
          из {series.total}
        </div>
        <div style={{
          fontFamily, fontSize: 44, fontWeight: 900, textTransform: "uppercase", color: "#111",
          background: "#FFDD00", display: "inline-block", padding: "10px 28px", borderRadius: 16,
...
const SeriesBadge: React.FC<{ series: Series }> = ({ series }) => (
  <AbsoluteFill style={{ pointerEvents: "none" }}>
    <div style={{
      position: "absolute", top: 158, right: 44, fontFamily, fontSize: 34, fontWeight: 900,
      color: "#111", background: "#FFDD00", padding: "6px 16px", borderRadius: 12,
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * POOL_FPS);
...
export const makePool = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * POOL_FPS);
  const Reel: React.FC = () => (
...
export const POOL4 = REEL_PLANS.map(makePool);
