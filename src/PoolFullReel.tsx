...
const msToFrame = (ms: number) => Math.round((ms / 1000) * PF_FPS);
const XFADE_MS = 350;
...
// createTikTokStyleCaptions иногда склеивает в одну страницу токены с большим разрывом
// (напр. через немую вставку между репликами) — режем такие страницы по паузе >700мс
const splitPagesByGap = (pages: TikTokPage[], maxGapMs: number): TikTokPage[] => {
  const out: TikTokPage[] = [];
  for (const page of pages) {
    let cluster: typeof page.tokens = [];
    for (const t of page.tokens) {
      if (cluster.length && t.fromMs - cluster[cluster.length - 1].toMs > maxGapMs) {
        out.push({ text: cluster.map((c) => c.text).join(""), startMs: cluster[0].fromMs, tokens: cluster });
        cluster = [];
      }
      cluster.push(t);
    }
    if (cluster.length) out.push({ text: cluster.map((c) => c.text).join(""), startMs: cluster[0].fromMs, tokens: cluster });
  }
  return out;
};
...
const Captions: React.FC<{ items: TimelineItem[] }> = ({ items }) => {
  const { fps } = useVideoConfig();
  const pages = useMemo(() => {
    const raw = createTikTokStyleCaptions({ captions: buildCaptions(items), combineTokensWithinMilliseconds: SWITCH_MS }).pages;
    return splitPagesByGap(raw, 700);
  }, [items]);
  return (<AbsoluteFill>{pages.map((page, i) => {
    const next = pages[i + 1] ?? null;
    const start = (page.startMs / 1000) * fps;
    // не тянуть страницу через тишину (немые вставки): максимум до конца последнего слова + небольшой запас
    const lastTokenEndMs = Math.max(...page.tokens.map((t) => t.toMs));
    const naturalEnd = ((lastTokenEndMs + 600) / 1000) * fps;
    const nextStart = next ? (next.startMs / 1000) * fps : start + fps * 3;
    const end = Math.min(nextStart, naturalEnd);
...
const HeroPhrase: React.FC<{ hero: Hero }> = ({ hero }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const durF = Math.round((hero.durMs / 1000) * PF_FPS);
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", pointerEvents: "none", opacity: op }}>
      ...
      <div style={{ textAlign: "center", transform: `translateY(${(1 - enter) * 40}px) scale(${interpolate(enter, [0, 1], [0.9, 1])})` }}>
        ...
        <div style={{ fontFamily, fontSize: 32, fontWeight: 900, textTransform: "uppercase", color: "#0a0a0a", background: "#FFE500", display: "inline-block", padding: "10px 26px", borderRadius: 12, marginTop: 26, rotate: "-2deg", boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>🏊 ВСЁ ПРО УХОД</div>
      </div>
    </AbsoluteFill>
  );
};
...
const Sticker: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * PF_FPS);
  const out = interpolate(frame, [durF - 8, durF], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
export const makePF = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * PF_FPS);
  const Reel: React.FC = () => (
...
export const POOLFULL = REEL_PLANS.map(makePF);
