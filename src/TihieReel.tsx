...
import plan from "./tihie-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * TIHIE_FPS);
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", opacity: Math.min(fadeIn, fadeOut) }}>
      <OffthreadVideo
        ...
        volume={item.volume}
        style={{ width: "100%", height: "100%", objectFit: "cover", transform: `scale(${zoom})`, filter: "contrast(1.04) saturate(1.06) brightness(1.06)" }}
      />
    </AbsoluteFill>
  );
};

// Без затемнения: убрали виньетку и чёрные полосы (по просьбе Романа), лёгкое зерно оставили
...
const SeriesCover: React.FC<{ series: Series }> = ({ series }) => {
  ...
  const op = interpolate(frame, [0, 8, 116, 130], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * TIHIE_FPS);
...
export const makeTihie = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * TIHIE_FPS);
  const Reel: React.FC = () => (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      ...
      {reel.series ? (
        <>
          <Sequence from={0} durationInFrames={135} name="series-cover"><SeriesCover series={reel.series} /></Sequence>
          <Sequence from={135} name="series-badge"><SeriesBadge series={reel.series} /></Sequence>
        </>
      ) : null}
...
export const TIHIE = REEL_PLANS.map(makeTihie);
