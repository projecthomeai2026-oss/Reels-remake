...
import plan from "./poolclean-plan.json";
...
type Outro = { title: string; sub?: string; cta?: string };
type ReelPlan = { id: string; title: string; music: string; items: PlanItem[]; series?: Series; accents?: Accent[]; gifs?: GifDef[]; outro?: Outro };
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * POOLCLEAN_FPS);
...
// Финишная CTA-плашка в конце ролика (поверх замедленного монтажа)
const Outro: React.FC<{ outro: Outro }> = ({ outro }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const enter = spring({ frame, fps, config: { damping: 15, stiffness: 150, mass: 0.7 } });
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", pointerEvents: "none", opacity: op }}>
      ...
      <div style={{ textAlign: "center", transform: `translateY(${(1 - enter) * 40}px) scale(${interpolate(enter, [0, 1], [0.92, 1])})` }}>
        {outro.sub ? (
          <div style={{ fontFamily, fontSize: 26, fontWeight: 800, letterSpacing: "0.32em", textTransform: "uppercase", color: "rgba(255,255,255,0.9)", marginBottom: 22 }}>
            {outro.sub}
          </div>
        ) : null}
        <div style={{ fontFamily, fontSize: 96, fontWeight: 900, fontStyle: "italic", color: "#fff", lineHeight: 0.95, textShadow: "0 10px 40px rgba(0,0,0,0.6)", textTransform: "uppercase" }}>
          {outro.title.split("\n").map((l, i) => <div key={i}>{l}</div>)}
        </div>
        {outro.cta ? (
          <div style={{
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * POOLCLEAN_FPS);
...
export const makePoolClean = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * POOLCLEAN_FPS);
  const Reel: React.FC = () => (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      ...
      {reel.outro ? (
        <Sequence from={Math.max(0, duration - 108)} durationInFrames={120} name="outro"><Outro outro={reel.outro} /></Sequence>
      ) : null}
    </AbsoluteFill>
  );
...
export const POOLCLEAN = REEL_PLANS.map(makePoolClean);
