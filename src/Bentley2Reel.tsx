...
import plan from "./bentley2-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * BENTLEY2_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * BENTLEY2_FPS);
...
export const makeBentley2 = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * BENTLEY2_FPS);
  const Reel: React.FC = () => (
...
export const BENTLEY2 = REEL_PLANS.map(makeBentley2);
