...
import plan from "./r2072-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * SW_FPS);
...
const GifOverlay: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * SW_FPS);
...
export const makeSW = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * SW_FPS);
  const Reel: React.FC = () => (
...
export const SMARTWIN = REEL_PLANS.map(makeSW);
