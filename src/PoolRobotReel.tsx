...
import plan from "./poolrobot-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * PR_FPS);
const XFADE_MS = 350;
...
const HeroPhrase: React.FC<{ hero: Hero }> = ({ hero }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const durF = Math.round((hero.durMs / 1000) * PR_FPS);
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", pointerEvents: "none", opacity: op }}>
      ...
      <div style={{ textAlign: "center", transform: `translateY(${(1 - enter) * 40}px) scale(${interpolate(enter, [0, 1], [0.9, 1])})` }}>
        ...
        <div style={{ fontFamily, fontSize: 32, fontWeight: 900, textTransform: "uppercase", color: "#0a0a0a", background: "#FFE500", display: "inline-block", padding: "10px 26px", borderRadius: 12, marginTop: 26, rotate: "-2deg", boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>🤖 РОБОТ-ПЫЛЕСОС</div>
      </div>
    </AbsoluteFill>
  );
};
...
const Sticker: React.FC<{ gif: GifDef }> = ({ gif }) => {
  ...
  const durF = Math.round((gif.durMs / 1000) * PR_FPS);
  const out = interpolate(frame, [durF - 8, durF], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
export const makePR = (reel: ReelPlan) => {
  const { items, totalMs } = buildTimeline(reel);
  const duration = Math.ceil((totalMs / 1000) * PR_FPS);
  const Reel: React.FC = () => (
...
export const POOLROBOT = REEL_PLANS.map(makePR);
