...
import plan from "./reels6-plan.json";
...
const msToFrame = (ms: number) => Math.round((ms / 1000) * REELS6_FPS);
      ...
      <OffthreadVideo
        ...
        style={{
          ...
          filter: item.hookLook
            ? "grayscale(1) contrast(1.28) brightness(1.06)"
...
// Киношный оверлей для холодного опенера: ч/б задаёт SegmentVideo,
// здесь — леттербокс-полосы и лёгкое зерно плёнки
    ...
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      ...
      <svg width="100%" height="100%" style={{ position: "absolute", inset: 0, opacity: 0.07 }}>
        <filter id="film-grain">
          <feTurbulence type="fractalNoise" baseFrequency="0.8" numOctaves="2" seed={frame} stitchTiles="stitch" />
          <feColorMatrix type="saturate" values="0" />
        </filter>
        <rect width="100%" height="100%" filter="url(#film-grain)" />
      </svg>
      {/* киношные полосы */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 150, backgroundColor: "#000" }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 150, backgroundColor: "#000" }} />
      {/* мягкое киновиньетирование */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      />
      {/* рамка съёмки: уголки видоискателя */}
      {[
        { top: 215, left: 55, borderWidth: "6px 0 0 6px" },
        { top: 215, right: 55, borderWidth: "6px 6px 0 0" },
        { bottom: 215, left: 55, borderWidth: "0 0 6px 6px" },
        { bottom: 215, right: 55, borderWidth: "0 6px 6px 0" },
      ].map((s, i) => (
        <div
          key={i}
          style={{
      ...
      <div
        style={{
          ...
          opacity: frame % 24 < 14 ? 1 : 0.25,
        }}
      >
        <div style={{ width: 30, height: 30, borderRadius: "50%", backgroundColor: "#FF3B30" }} />
        REC
      </div>
      <div
        style={{
      ...
      >
        {`0:00:${String(Math.floor(frame / 30)).padStart(2, "0")}`}
      </div>
    </AbsoluteFill>
  );
};
...
export const makeReel = (reel: ReelPlan) => {
  ...
  const duration = Math.ceil((totalMs / 1000) * REELS6_FPS);

  // Стикер с названием — на старте основной части (первый flash-элемент),
  // если его нет (нет холодного опенера) — с 8-го кадра
  const firstFlash = items.find((it) => it.flash);
...
export const REELS6 = REEL_PLANS.map(makeReel);
